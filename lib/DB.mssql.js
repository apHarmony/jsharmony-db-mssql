...
DBdriver.prototype.getCapabilities = function(){
  return ['recordset','multirecordset','row','command','scalar','bulk_insert','bulk_create'];
};
...
DBdriver.prototype.Exec = function (dbtrans, context, return_type, sql, ptypes, params, callback, dbconfig) {
  if(!dbconfig) throw new Error('dbconfig is required');
  if(!_.includes(['recordset','multirecordset','row','command','scalar','bulk_insert','bulk_create'], return_type)) throw new Error('DB operation not supported: '+(return_type||'').toString());
  var _this = this;

  var bulk = ((return_type == 'bulk_insert')||(return_type == 'bulk_create'));
  var contextSQL = _this.getContextSQL(context);
  if(bulk){
    if(contextSQL) throw new Error("Set _DBContext to empty string for bulk operation");
    if(!sql.name) throw new Error("Bulk operation requires 'name' parameter");
    if(!sql.columns) throw new Error("Bulk operation requires 'columns' parameter");
    if(!sql.data) throw new Error("Bulk operation requires 'data' parameter");
  }
  ...
  _this.ExecSession(dbtrans, dbconfig, function (err, con, presql, conComplete) {
    ...
    var execsql = null;
    if(bulk){
      var table = new (_this.mssql()).Table(sql.name);
      table.create = (return_type == 'bulk_create');
      // Options: ['nullable', 'primary', 'identity', 'readOnly', 'length']
      _.each(sql.columns, function(column){ table.columns.add(column.name, _this.getDBType(column.type), column.options); });
      _.each(sql.data, function(row){ table.rows.add.apply(table.rows, row); });
      execsql = table;
    }
    else {
      execsql = presql + (sql||'').toString();
      execsql = _this.applySQLParams(execsql, ptypes, params);

      //Add context SQL
      execsql = contextSQL + execsql;
    }
